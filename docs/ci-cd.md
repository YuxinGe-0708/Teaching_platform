...
- Pull requests run Maven unit tests, build both images, start a clean MySQL/Backend/Frontend Compose environment, apply the ordered SQL files in `db/migrations/`, verify the migration ledger, and run the container smoke and business regression scripts.
...
`k8s-deploy.sh` applies the namespace, ConfigMap, Secret, MySQL PVC/Deployment, backend PVC/Deployment, and frontend Service. After MySQL is ready it applies each SQL file in `db/migrations/` once, recording completed versions in `schema_migrations`. In kind mode it enables repeatable test data, maps the frontend NodePort to runner port 3000, waits for every rollout, checks `/healthz`, renders `/login`, and runs the existing login/session smoke test. Any failure exits non-zero.
