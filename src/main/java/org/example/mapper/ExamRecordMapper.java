...
public interface ExamRecordMapper {
    ...
    @Select("SELECT e.*, u.name AS student_name, t.title AS task_title FROM exam_record e LEFT JOIN user u ON e.student_id = u.id LEFT JOIN task t ON e.task_id = t.id WHERE e.student_id = #{studentId} AND e.task_id = #{taskId}")
    ExamRecord findByStudentAndTask(@Param("studentId") Long studentId, @Param("taskId") Long taskId);

    @Select("SELECT e.*, u.name AS student_name, t.title AS task_title FROM exam_record e LEFT JOIN user u ON e.student_id = u.id LEFT JOIN task t ON e.task_id = t.id WHERE e.task_id = #{taskId} ORDER BY e.submit_time DESC")
    List<ExamRecord> findByTaskId(Long taskId);

    @Select("SELECT e.*, u.name AS student_name, t.title AS task_title FROM exam_record e LEFT JOIN user u ON e.student_id = u.id LEFT JOIN task t ON e.task_id = t.id WHERE e.student_id = #{studentId} ORDER BY e.created_at DESC")
