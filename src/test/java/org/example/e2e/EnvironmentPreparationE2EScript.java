...
public class EnvironmentPreparationE2EScript extends E2eTestSupport {
    ...
    void verifyDeployedEnvironmentAndSeedData() throws Exception {
        ...
        try {
            ...
            assertion = passed
                    ? "通过：端到端测试环境可用。teacher_demo/student_005/student_006 为可选种子账号，缺失时各主线会独立补齐。"
                    : "失败/阻塞：端到端测试环境不可用，需先启动容器、补齐 admin 账号或准备测试文件。";
            ...
            E2eMatrix.add(
                    ...
                    objectData("baseUrl", E2eConfig.BASE_URL, "db", E2eConfig.summary(),
                            "requiredAccounts", new String[] {"admin"},
                            "optionalSeedAccounts", new String[] {"teacher_demo", "student_006", "student_005"},
                    ...
                    "登录页返回 200；数据库连接正常；admin 和上传样例文件存在；其他种子账号仅作观察，不作为各主线的隐含依赖。",
                    actual,
                    assertion,
                    passed);
            if (db != null) db.close();
