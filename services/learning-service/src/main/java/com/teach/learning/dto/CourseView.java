...
public class CourseView {
    ...
    private String status; private Timestamp createdAt;
    public static CourseView from(Course c) {
        CourseView v = new CourseView(); v.id = c.getId(); v.name = c.getName(); v.code = c.getCode();
        v.description = c.getDescription(); v.credits = c.getCredits(); v.subjectCategory = c.getSubjectCategory();
        v.hours = c.getHours(); v.teacherId = c.getTeacherId(); v.inviteCode = c.getInviteCode();
        v.allowJoin = c.getAllowJoin(); v.status = c.getStatus(); v.createdAt = c.getCreatedAt();
        return v;
    }
