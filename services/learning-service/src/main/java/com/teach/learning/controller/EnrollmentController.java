...
public class EnrollmentController {
    ...
    public ApiResponse<List<EnrollmentView>> studentEnrollments(@PathVariable Long studentId) {
    ...
    public ApiResponse<List<EnrollmentView>> courseEnrollments(@PathVariable Long courseId) {
    ...
    public ApiResponse<EnrollmentView> enroll(@RequestParam Long studentId, @RequestParam Long courseId,
        ...
        CourseEnrollment enrollment = enrollmentService.enroll(studentId, courseId, classId);
        if (enrollment == null) return ApiResponse.fail("选课失败，课程不存在或不允许加入");
        return ApiResponse.ok(EnrollmentView.from(enrollment));
    }
