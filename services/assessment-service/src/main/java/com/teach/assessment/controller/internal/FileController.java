package com.teach.assessment.controller.internal;
...
public class FileController {
    ...
    public FileController(@Value("${app.upload-path:${APP_UPLOAD_PATH:uploads}}") String configured) { this.root = Paths.get(configured).toAbsolutePath().normalize(); }

    @PostMapping("/submissions")
    public ApiResponse<String> uploadSubmission(@RequestParam("file") MultipartFile file) { return upload(file, "submissions"); }
    ...
    private ApiResponse<String> upload(MultipartFile file, String... dirs) {
        ...
        try {
            Path dir = root; for (String d : dirs) dir = dir.resolve(d).normalize();
    ...
    private Path resolve(String value) {
        String p = value == null ? "" : value.replace('\\', '/'); int marker = p.indexOf("uploads/"); if (marker >= 0) p = p.substring(marker + 8);
        Path resolved = root.resolve(Paths.get(p).normalize()).normalize(); if (!resolved.startsWith(root)) throw new IllegalArgumentException("path outside uploads"); return resolved;
    }
}
